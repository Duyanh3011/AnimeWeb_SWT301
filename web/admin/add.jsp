                            ...
                            <div class="login__form">
                                ...
                                <form id="form" action="add" method="post">
                                    ...
                                    <c:forEach items="${requestScope.genre}" var="g">
                                        <option name="Genre${g.id}" value="${g.id}" ${g.id == seleGenID ? 'selected' : ''}>${g.name}</option>
                                    </c:forEach>
                                </select>
                                <select name="seleStudio" >
                                    <option value="0">All</option>
                                    <c:forEach items="${requestScope.studio}" var="g">
                                        <option name="Studio${g.id}" value="${g.id}" ${g.id == seleStuID ? 'selected' : ''}>${g.name}</option>
                                    </c:forEach>
                                </select>

                                <button style="display: block" type="submit" class="site-btn">Add Now</button>
                                <h3 style="color: green" id="result">${requestScope.mes}</h3>
                            </form>
                        </div>
                    </div>
